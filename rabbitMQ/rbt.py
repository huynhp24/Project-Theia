...
DEST = config['default']['image_upload_folder']
SOURCEDIR = config['default']['sourcedir']
...
def checkingImgURL(img, uuid):
    try:
        ...
        if (img.find('.png') or img.find('.jpg')):
            ...
            # l.warning(" File not ending with jpg or png ")
            # l.debug("super fast loop info data")
            # l.info("App started, this task is started")
            # l.warning("file name is not super good")
            # l.error("an error occured here")
            # l.exception("an exception occured here")
def receive(rmq_q):
