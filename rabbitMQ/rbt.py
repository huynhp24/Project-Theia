...
import shutil
import requests
sys.path.insert(1,'/xxxx/testTheia/serverside')
import labels, textdetect
...
DEST = '/xxxx/testTheia/rabbitMQ/Images'
SOURCEDIR = '/xxxx/testTheia/rabbitMQ'


def storeToDB(imgFile, id):
    conn = mysql.connector.connect(user='xxxx', password='xxxx',
                                   host='127.0.0.1',
                                   database='xxx')
    cur = conn.cursor()
    ...
    tsql = "insert into jsondata(uuid, image_Location, label_list, detect_text, file_date) values (%s, %s, %s, %s, %s)"
    cur.execute(tsql, (id, imgS3Location, labelJson, textJson, filestamp))
    conn.commit()
