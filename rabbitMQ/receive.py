...
def main():
    ...
    def checkingImgURL(img, req):
        try:
            ...
            # the exception only catch error when the url does not have '.png' or '.jpg' word in the url
            # therefore, have to make sure the image extension ('.png' or '.jpg') appears at the end of the url
            if (img.endswith('.png') or img.endswith('.jpg')):
                with open(imgFile, 'wb') as f:
                    f.write(webpage)
                imgurl = Image.open(imgFile)
                # move the image file to imgPathToS3 function to run rekognition
                imgPathToS3(imgFile)
                imgurl.close()
